...
namespace CodeCompletion
{
	...
	public class DomSyntaxTreeVisitor : AbstractVisitor
    {
        ...
        public override void visit(unit_module _unit_module)
        {
            ...
            if (CodeCompletionController.comp != null && CodeCompletionController.comp.CompilerOptions.CurrentProject != null && CodeCompletionController.comp.CompilerOptions.CurrentProject.ContainsSourceFile(_unit_module.FileName))
            {
                ...
                if (_unit_module.compilerDirectives == null)
                    _unit_module.compilerDirectives = new List<PascalABCCompiler.SyntaxTree.compiler_directive>();
                foreach (IReferenceInfo ri in refs)
                {
                    _unit_module.compilerDirectives.Add
            ...
            if (_unit_module.compilerDirectives != null)
                foreach (PascalABCCompiler.SyntaxTree.compiler_directive dir in _unit_module.compilerDirectives)
                {
        ...
        public override void visit(program_module _program_module)
        {
            ...
            if (CodeCompletionController.comp != null && CodeCompletionController.comp.CompilerOptions.CurrentProject != null && CodeCompletionController.comp.CompilerOptions.CurrentProject.ContainsSourceFile(_program_module.FileName))
            {
                ...
                if (_program_module.compilerDirectives == null)
                    _program_module.compilerDirectives = new List<PascalABCCompiler.SyntaxTree.compiler_directive>();
                foreach (IReferenceInfo ri in refs)
                {
                    _program_module.compilerDirectives.Add
                        (new PascalABCCompiler.SyntaxTree.compiler_directive(new token_info("reference"), new token_info(ri.FullAssemblyName)));
                }
            }
            List<string> included_files = new List<string>();
            if (_program_module.compilerDirectives != null)
                foreach (PascalABCCompiler.SyntaxTree.compiler_directive dir in _program_module.compilerDirectives)
                {
#if DEBUG
                    // SSM test 05.08.17
                    if (dir == null)
                        File.AppendAllText("log.txt", "dir == null" + Environment.NewLine + _program_module.compilerDirectives.Count + Environment.NewLine);
#endif
                    if (dir.Name.text.ToLower() == "reference")
                    {
                        try
        ...
        public override void visit(c_module _c_module)
        {
            ...
            //List<Scope> netScopes = new List<Scope>();
            //PascalABCCompiler.NetHelper.NetScope ns=new PascalABCCompiler.NetHelper.NetScope(unl,_as,tcst);
            if (_c_module.compilerDirectives != null)
                foreach (PascalABCCompiler.SyntaxTree.compiler_directive dir in _c_module.compilerDirectives)
                {
