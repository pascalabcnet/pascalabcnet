...
SosediB[i1,j1] := SosediB[i1,j1]+1;
  SosediB[i1,j]  := SosediB[i1,j]+1;
  SosediB[i1,j2] := SosediB[i1,j2]+1;
  SosediB[i,j1] := SosediB[i,j1]+1;
  SosediB[i,j2] := SosediB[i,j2]+1;
  SosediB[i2,j1] := SosediB[i2,j1]+1;
  SosediB[i2,j] := SosediB[i2,j]+1;
  SosediB[i2,j2] := SosediB[i2,j2]+1;
end;
...
//  yc:=1;
  ...
  {for i:=xc-10 to xc+10 do
  for j:=yc-10 to yc+10 do
  if random(10)>3 then
    SetCell(i,j);{}
  SetCell(xc,yc);
  SetCell(xc,yc+1);
...
procedure NextGen;
var
  i,j,ik,jk,ik1,jk1,ik2,jk2,ifirst,jfirst,ilast,jlast,s: integer;
