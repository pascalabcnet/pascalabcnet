...
begin  
  ...
  var ScreenBuffer: array of array of byte := new array of byte[size+1];
  ...
  repeat
    ...
    for I:=0 to 5 do begin
      ...
      ScreenBuffer[y-1,x-1] := s;
      ScreenBuffer[y+1,x  ] := s;
      SetPixel(y-1,x+1,ColorsTable[s]);
      SetPixel(y-1,x,  ColorsTable[s]);
      SetPixel(y-1,x-1,ColorsTable[s]);
      SetPixel(y+1,x,  ColorsTable[s]);
    end;
