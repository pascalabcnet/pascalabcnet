...
namespace PascalABCCompiler.SyntaxTree
{
	...
	public class SyntaxTreeStreamWriter:IVisitor
	{
		...
		{
			write_expression(_expression_list);
		...
		{
			bw.Write((Int16)38);
			write_typed_parametres(_typed_parametres);
		}

		public void write_typed_parametres(typed_parameters _typed_parametres)
		{
			write_declaration(_typed_parametres);
			if (_typed_parametres.idents == null)
			{
				bw.Write((byte)0);
			}
			else
			...
			bw.Write((byte)_typed_parametres.param_kind);
		...
		{
			bw.Write((Int16)39);
			write_formal_parametres(_formal_parametres);
		}

		public void write_formal_parametres(formal_parameters _formal_parametres)
		{
			write_syntax_tree_node(_formal_parametres);
			if (_formal_parametres.params_list == null)
			{
				bw.Write((byte)0);
			}
			else
			{
				bw.Write((byte)1);
				bw.Write(_formal_parametres.params_list.Count);
				for(Int32 ssyy_i = 0; ssyy_i < _formal_parametres.params_list.Count; ssyy_i++)
				{
					if (_formal_parametres.params_list[ssyy_i] == null)
					{
						bw.Write((byte)0);
					}
					else
					{
						bw.Write((byte)1);
						_formal_parametres.params_list[ssyy_i].visit(this);
					}
				}
			}
		}
		...
		{
			write_expression(_pascal_set_constant);
		...
		{
			bw.Write((Int16)145);
			write_oberon_import_module(_oberon_import_module);
		}

		public void write_oberon_import_module(oberon_import_module _oberon_import_module)
		{
			write_unit_or_namespace(_oberon_import_module);
			...
			{
				bw.Write((byte)1);
				_oberon_import_module.new_name.visit(this);
			}
		}
		...
		{
			bw.Write((Int16)164);
			write_type_definition_list(_type_definition_list);
		}

		public void write_type_definition_list(type_definition_list _type_definition_list)
		{
			write_syntax_tree_node(_type_definition_list);
			...
			{
				bw.Write((byte)1);
				bw.Write(_type_definition_list.defs.Count);
				for(Int32 ssyy_i = 0; ssyy_i < _type_definition_list.defs.Count; ssyy_i++)
				{
					if (_type_definition_list.defs[ssyy_i] == null)
					{
						bw.Write((byte)0);
					}
					else
					{
						bw.Write((byte)1);
						_type_definition_list.defs[ssyy_i].visit(this);
					}
				}
			}
		}
		...
		{
			write_expression(_question_colon_expression);
		...
		{
			write_expression(_default_operator);
		...
		{
			write_expression(_bracket_expr);
        ...
        public void write_expression(function_lambda_definition _function_lambda_definition)
        {
            write_statement(_function_lambda_definition);
        }
        ...
        public void write_expression(function_lambda_call _function_lambda_call)
        {
            write_statement(_function_lambda_call);
        }
