        ...
        private void FixResultVar()
        {
            for (int i=0; i<Variables.Count; ++i)
                if (Variables[i].name.StartsWith("$rv_"))
                {
                    ...
                    TreeRealization.local_variable NewResult = new PascalABCCompiler.TreeRealization.local_variable(StringConstants.result_variable_name, OldResult.type, OldResult.function, OldResult.loc);
