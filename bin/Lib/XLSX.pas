...
begin
  var elms := Load(path);
  var (LU,RD) := elms.Item['dimension'].GetAttribute('ref').Split(':').Sel(s->s.ToCR);
  var (w,h) := (RD[1]-LU[1]+1,RD[0]-LU[0]+1);
  var sheet : array of array of string;
  setLength(sheet,h);
  for var i:=0 to h-1 do sheet[i] := new string[w];

  foreach var node:XmlNode in elms.Item['sheetData'].ChildNodes do begin
    foreach var cell:XmlNode in node.ChildNodes do begin
...
begin
  ...
  foreach var node:XmlNode in elms.Item['sheets'].ChildNodes do begin
