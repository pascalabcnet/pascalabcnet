...
/* это нужно дл€ intellisensа чтобы строилось дерево при отсутствии точки в конце */
optional_tk_point 
    : tkPoint
        { $$ = $1; }
    | tkSemiColon
...
func_decl_lambda
	: identifier tkArrow lambda_function_body
		{
			...
			//if (sl.expr_lambda_body || SyntaxVisitors.HasNameVisitor.HasName($3, "Result") != null) // если это было выражение или есть переменна€ Result, то автовывод типа 
			    $$ = new function_lambda_definition(lambdaHelper.CreateLambdaName(), formalPars, new lambda_inferred_type(new PascalABCCompiler.TreeRealization.lambda_any_type_node(), @1), $3 as statement_list, @$);
			//else 
    ...
    | tkRoundOpen tkRoundClose lambda_type_ref_noproctype tkArrow lambda_function_body
		{
		    // «десь надо анализировать по телу и либо оставл€ть lambda_inferred_type, либо делать его null!
