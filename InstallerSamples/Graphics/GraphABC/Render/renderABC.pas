...
procedure FastDrawColorBitmap(data: array [,] of System.Drawing.Color);
type 
  PRGB = ^uint32;
var
...
begin
  ...
  try
    ptr := bitmapData.Scan0;//.ToPointer;
    pixelSize := 4; // 4 байта на пиксель (ARGB)
    
    for var y := 0 to height - 1 do
      for var x := 0 to width - 1 do
      begin
        var offset := y * bitmapData.Stride + x * pixelSize;
        var color := data[x, y];
        
        PRGB(pointer(ptr + offset))^ := color.ToArgb;     
      end;
  finally
    bmp.UnlockBits(bitmapData);
  end;
  
  GraphWindowGraphics.DrawImage(bmp, 0,0);
  bmp.Dispose;
end; 

procedure DrawPixelsABC(a: array [,] of Color);
begin
  FastDrawColorBitmap(a);
end;
...
begin
  ...
  Window.Title := 'Enhanced Ray Tracer - Beautiful Scene ABC';
  ...
  Println($'Rendering {width}x{height} scene...');
  MillisecondsDelta;
  var pixels := Render(width, height, spheres, lights);
  var m := MillisecondsDelta;
  
  DrawPixelsABC(pixels);
  TextOut(5, 5, $'Rendered in {m} ms');
  TextOut(5, 25, $'Resolution: {width}x{height} (3x3 AA)');
  TextOut(5, 45, $'Objects: {spheres.Length} spheres, {lights.Length} lights');
end.