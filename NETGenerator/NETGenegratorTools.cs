...
namespace PascalABCCompiler.NETGenerator
{
    ...
    class NETGeneratorTools
    {
                ...
                default:
                    if (IsPointer(elem_type))
                        ...
                        if (elem_type.IsValueType)//если это структура
                        {
                            ...
                            // проверки нужно ли замен€ть тип возвр. знач. метода get_val массива на указатель
        ...
        public static void PushLdc(ILGenerator il, Type elem_type, object value)
        {
            switch (Type.GetTypeCode(elem_type))
            {
                ...
                case TypeCode.UInt64:
                    ...
                    {
                        //Ёто будет медленно работать. Ќадо переделать.
                        //Ќадо разобратьс€ как сссделано в C#, там все нормально
                ...
                default:
                    if (IsEnum(elem_type))
                        ...
                        throw new Exception("Ќемогу положить PushLdc дл€ " + value.GetType().ToString());
                    break;
            }
        }
        ...
        {
            if (IsPointer(tp))  //INTPTR TODO «десть проблема с Unbox_Any
                return;
            //(ssyy) ¬ставил 15.05.08
