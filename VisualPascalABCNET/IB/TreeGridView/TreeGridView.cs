...
namespace AdvancedDataGridView
{
	...
	{		
		...
		internal VisualStyleRenderer _rOpen;
		internal VisualStyleRenderer _rClosed;

        public VisualStyleRenderer rOpen
        {
            get
            {
                if (_rOpen == null)
                _rOpen = new VisualStyleRenderer(VisualStyleElement.TreeView.Glyph.Opened);
                return _rOpen;
                //rClosed = new VisualStyleRenderer(VisualStyleElement.TreeView.Glyph.Closed);
            }
        }

        public VisualStyleRenderer rClosed
        {
            get
            {
                if (_rClosed == null)
                    _rClosed = new VisualStyleRenderer(VisualStyleElement.TreeView.Glyph.Closed);
                return _rClosed;
            }
        }
...
//				}
        ...
        protected override void OnAllowUserToAddRowsChanged(EventArgs e)
        {
            ...
            //{
            //    TreeGridNode node = new TreeGridNode(this);
            //    this.Nodes.Add(node);
            //    SiteNode(node);
            //}
        }
        ...
        private void SetVerticalParent(Control ctrl)
        {
        	this.VerticalScrollBar.Parent = ctrl;
        }
        
        delegate void Func1(Control ctrl);
		...
		private void LockVerticalScrollBarUpdate(bool lockUpdate/*, bool delayed*/)
		{
            ...
            if (!this._inExpandCollapse)
            {
                if (lockUpdate)
                {
                    ...
                    Func1 f = SetVerticalParent;
                    this.VerticalScrollBar.Invoke(f,hideScrollBarControl);
                }
                else
                {
                    //this.VerticalScrollBar.Parent = this;
                    Func1 f = SetVerticalParent;
                    this.VerticalScrollBar.Invoke(f,this);
                }
            }
        }
