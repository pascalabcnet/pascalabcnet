...
//				bgColorBrush.Dispose();
			...
			IList<TextMarker> markers=null;
			try
			{
			Brush brush = GetMarkerBrushAt(Document.GetLineSegment(lineNumber).Offset,0,ref col, out markers);
			if (brush == BrushRegistry.GetBrush(Color.Red))
			{
				red = true;
			}
			else if (brush == BrushRegistry.GetBrush(Color.Yellow))
			{
				yellow = true;
			}
			else if (brush == BrushRegistry.GetBrush(Color.OrangeRed))
			{
				orange_red = true;
			}
			}
			...
			if (lineNumber < textArea.Document.TotalNumberOfLines) {
				...
				if (yellow) fillBrush = BrushRegistry.GetBrush(Color.Yellow);
				else if (red) fillBrush = BrushRegistry.GetBrush(Color.Red);
				else if (orange_red) fillBrush = BrushRegistry.GetBrush(Color.OrangeRed);
...
//			bgColorBrush.Dispose();
		...
		{
			List<MarkerToDraw> lst = new List<MarkerToDraw>();
			RectangleF res_rect = RectangleF.Empty;
			foreach (MarkerToDraw m in markersToDraw) {
				...
				switch (marker.TextMarkerType) {
					...
					case TextMarkerType.SolidBlock:
						...
						break;
					case TextMarkerType.Cant:
//						Pen p = new Pen(Color.Gray);
//						p.DashStyle = System.Drawing.Drawing2D.DashStyle.Dot;
//						g.DrawRectangle(p,drawingRect.X,drawingRect.Y,drawingRect.Width,drawingRect.Height);
						if (!res_rect.IsEmpty)
						res_rect = RectangleF.Union(res_rect,drawingRect);
						else
						res_rect = drawingRect;
						break;
				}
			}
			if (!res_rect.IsEmpty)
			{
				Pen p = new Pen(Color.Gray);
				p.DashStyle = System.Drawing.Drawing2D.DashStyle.Dot;
				g.DrawRectangle(p,res_rect.X,res_rect.Y,res_rect.Width,res_rect.Height);
			}
			markersToDraw.Clear();
		}
		...
		Brush GetMarkerBrushAt(int offset, int length, ref Color foreColor, out IList<TextMarker> markers)
		{
			...
			foreach (TextMarker marker in markers) {
                ...
                else if (marker.TextMarkerType == TextMarkerType.Underlined)
                {
                    foreColor = marker.Color;
                    return BrushRegistry.GetBrush(Color.White);
                }
			}
			return null;
		}
...
//				if (wordBackBrush == BrushRegistry.GetBrush(Color.Yellow))
//					yellow = true;
//				else if (wordBackBrush == BrushRegistry.GetBrush(Color.Red))
//					red = true;
//				else if (wordBackBrush == BrushRegistry.GetBrush(Color.OrangeRed))
//					orange_red = true;
				...
				if (currentWord.Length > 1) {
					...
					foreach (TextMarker marker in markers) {
						if (marker.TextMarkerType == TextMarkerType.SolidBlock)
						{
//							if (marker.Color == Color.Yellow)
//								yellow = true;
//							else if (marker.Color == Color.Red)
//								red = true;
//							else if (marker.Color == Color.OrangeRed)
//								orange_red = true;
							//g.FillRectangle(BrushRegistry.GetBrush(marker.Color),new RectangleF(lineRectangle.X,lineRectangle.Y,700,300));
						}
				...
				if (currentWord.Type == TextWordType.Space) {
                    ...
                    Brush tmp_brush = wordBackBrush;
                    
                    //modified
                    if (highlight != null && wordBackBrush != selectionBackgroundBrush)
                    {
                            ...
                            highlight.CloseBrace.Y == lineNumber && highlight.CloseBrace.X == currentWordOffset)
                        {
                            ...
                            wordBackBrush = BrushRegistry.GetBrush(System.Drawing.Color.FromArgb(219, 224, 204));
                        }
                    }
                    //end modified
...
//			Pen p = new Pen(System.Drawing.Color.Gray);
//			p.DashStyle = System.Drawing.Drawing2D.DashStyle.Dot;
//			g.DrawRectangle(p,new Rectangle(position.X, position.Y, wordWidth-1, FontHeight-1));
			DrawString(g,
			           word,
			           font,
			           foreColor,
		...
		{
            g.FillRectangle(BrushRegistry.GetBrush(System.Drawing.Color.FromArgb(219, 224, 204)), rect);
